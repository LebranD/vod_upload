...
class TXUGCPublish {
  ...
  static const _PUBLISH_METHOD_CHANNEL_PATH = "cloud.tencent.com/txvodplayer/videoUpload";
      ...
      case Apis.ON_PUBLISH_PROGRESS:
        {
          ...
          TXUGCPublishLog.debug(_TAG, "onPublishProgress : id : $taskId : uploadBytes : ${data["uploadBytes"]} totalBytes : ${data["totalBytes"]}");
      ...
      case Apis.ON_MEDIA_PUBLISH_PROGRESS:
        {
          ...
          TXUGCPublishLog.debug(_TAG, "onMediaPublishProgress : id : $taskId : uploadBytes : ${data["uploadBytes"]} totalBytes : ${data["totalBytes"]}");
  ...
  Future<int> publishVideo(TXPublishParam param) async {
    TXUGCPublishLog.debug(_TAG, "publishVideo : id : $id : param : ${param.toString()}");
  ...
  Future<int> publishMedia(TXMediaPublishParam param) async {
    TXUGCPublishLog.debug(_TAG, "publishMedia : id : $id : param : ${param.toString()}");
    bool isDefaultResumeController = _uploadResumeController == null;
  ...
  Future<int> resumeUploadVideo(TXPublishParam param) async {
    TXUGCPublishLog.debug(_TAG, "resumeUploadVideo : id : $id : param : ${param.toString()}");
    bool isDefaultResumeController = _uploadResumeController == null;
  ...
  Future<int> resumeUploadMedia(TXMediaPublishParam param) async {
    TXUGCPublishLog.debug(_TAG, "resumeUploadMedia : id : $id : param : ${param.toString()}");
    bool isDefaultResumeController = _uploadResumeController == null;
  ...
  Future<void> setAppId(int appId) async {
    var respJson = await _apiChannel.invokeMethod(Apis.SET_APPID, {"id": id, "appId": appId});
  ...
  Future<ReportInfo?> getStatusInfo() async {
    var respJson = await _apiChannel.invokeMethod(Apis.GET_STATUS_INFO, {"id": id});
    var resp = jsonDecode(respJson);
    if (resp["code"] == 0) {
      ReportInfo reportInfo = ReportInfo.formJson(resp["data"]);
      TXUGCPublishLog.debug(_TAG, "getStatusInfo : id : $id : reportInfo : ${reportInfo.toString()}");
      return reportInfo;
    }
    return null;
  }
  ...
  /// prepare upload
  static Future<void> prepareUpload(String signature, IPrepareUploadCallback callback) async {
    TXUGCPublishLog.debug(_TAG, "prepareUpload : signature : $signature");
    callback.onLoading();
    await _apiChannel.invokeMethod(Apis.PREPARE_UPLOAD, {"signature": signature});
    callback.onFinish();
  }
    ...
    switch (call.method) {
      case Apis.SAVE_SESSION:
        ...
        if (defaultTargetPlatform == TargetPlatform.android) {
          _uploadResumeController?.saveSessionAndroid(filePath, vodSessionKey, uploadId, uploadInfo);
        } else if (defaultTargetPlatform == TargetPlatform.iOS) {
          _uploadResumeController?.saveSessionIOS(filePath, vodSessionKey, resumeData, uploadInfo);
        }
        break;
      ...
      case Apis.IS_RESUME_UPLOAD_VIDEO:
        ...
        var uploadJson = call.arguments["uploadInfo"];
        var uploadInfo = TVCUploadInfo.fromJson(jsonDecode(uploadJson));
        res = _uploadResumeController?.isResumeUploadVideo(uploadId, uploadInfo, vodSessionKey, fileLastModTime, coverFileLastModTime);
        break;
    }
    if (res == null) return;
